...
-- 단일 행 연산자 : 주로 비교 연산자(=, >, <, >=, <=, <>(같지않다))를 사용하는 경우 하나의 행만 반환해야
-- 이런 경우에는 다중 행 연사자를 사용해야 합니다.
SELECT * FROM employees
WHERE job_id = (SELECT job_id FROM employees
...
-- first_name이 Davide인 사람들의 급여와 같은 급여를 받는 사람들을 조회 
SELECT * FROM employees
...
--     250 -> false, 200qheksms zmwlaks 300, 400보다는 크지 않으므로.)
SELECT * FROM employees
...
-- 서브쿼리 내에서 jh에 있는 id와 edp dlTsms id가 일치할 때마다 1이라는 값을 조회
-- EIXIST 연산자가 1이 조회가 될 때 데이터가 존재한다는 것을 판단하여
